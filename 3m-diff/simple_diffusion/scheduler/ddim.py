...
class DDIMScheduler:
    ...
    def generate(
        ...
        device=None,
        save_intermediate=False,  # Add this flag to control whether to save intermediate images
        output_dir=None,  # Add this to specify the directory to save intermediate images
    ):
        ...
        for i, t in enumerate(tqdm(self.timesteps)):
            ...
            if save_intermediate:
                ...
                intermediate_image = unnormalize_to_zero_to_one(image)
                intermediate_images.append(intermediate_image.cpu().permute(0, 2, 3, 1).numpy())
                
                # Save intermediate image to disk
                if output_dir:
                    os.makedirs(output_dir, exist_ok=True)
                    for idx, img in enumerate(intermediate_image):
                        img = Image.fromarray((img.cpu().permute(1, 2, 0).numpy() * 255).astype(np.uint8))
                        img.save(f"{output_dir}/step_{i:04d}_sample_{idx:04d}.jpeg")

        final_image = unnormalize_to_zero_to_one(image)
        return {
            "sample": final_image.cpu().permute(0, 2, 3, 1).numpy(),
            "sample_pt": final_image,
            "intermediate_samples": intermediate_images if save_intermediate else None,
        }
