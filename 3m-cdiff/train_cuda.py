...
# DataLoader setup
batch_size = 8
kwargs = {'num_workers': 0, 'pin_memory': True} if device.type == 'cuda:3' else {}
folder = "cuda3"
...
def show_images(images, labels, num_images=8):
    ...
    plt.tight_layout()
    plt.show()
    plt.savefig("cuda3/data_loader1.png",dpi=600)
    plt.close()
...
plt.savefig("cuda3/data_loader2.png",dpi=600)
plt.close()
...
def save_checkpoint(epoch, model, optimizer, losses, path="checkpoints_cuda3"):
...
bsbs = 4 # reduced batch size for learning stability
...
checkpoint_dir = "checkpoints_cuda3"
os.makedirs(checkpoint_dir, exist_ok=True)

# Optional: Load from checkpoint if resuming training
start_epoch = 0
last_epoch = 0
if os.path.exists("checkpoints_cuda3/checkpoint_epoch_{}.pth".format(last_epoch)):
    start_epoch, losses = load_checkpoint("checkpoints_cuda3/checkpoint_epoch_{}.pth".format(last_epoch), net, opt)
...
for epoch in range(last_epoch+1, n_epochs):
    ...
    # View the loss curve
    plt.plot(losses)
    plt.savefig("cuda3/loss_plot"+str(epoch)+".png",dpi=600)
    plt.close()
    if epoch % 15 == 0:
        ...
        plt.tight_layout()
        plt.savefig(f"cuda3/generated{epoch}.png", dpi=600, bbox_inches='tight')
        plt.close()

        # Create directory structure and save individual images
        epoch_dir = f"cuda3/epoch{epoch}"
